...
import { loadPoems, savePoems, generatePoemId, validatePoem, type Poem } from '@/lib/poem-data'
  ...
  try {
    ...
    if (action === 'create') {
      ...
      if (data.poems.some(p => p.id === newPoem.id)) {
        return NextResponse.json({ errors: ['同じIDのポエムが既に存在します'] }, { status: 400 })
      }

      data.poems.push(newPoem)
      data.updatedAt = new Date().toLocaleDateString('ja-JP', { 
        year: 'numeric', 
        month: '2-digit', 
        day: '2-digit',
        timeZone: 'Asia/Tokyo'
    ...
    if (action === 'update') {
      ...
      if (index === -1) {
        return NextResponse.json({ errors: ['ポエムが見つかりません'] }, { status: 404 })
      }
    ...
    return NextResponse.json({ errors: ['不正なアクションです'] }, { status: 400 })
  } catch (error) {
