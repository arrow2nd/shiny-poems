...
export default function PoemAdmin() {
  ...
  const suggestions = useMemo(() => {
    ...
    const clothesTitles = [
      ...new Set(data.poems.map((p) => p.clothesTitle))
    ].sort();
    const clothesNames = [
      ...new Set(data.poems.map((p) => p.clothesName))
    ].sort();
  ...
  useEffect(() => {
    if (data) {
      ...
      const filtered = data.poems.filter(
        (poem) =>
    ...
    if (e.key === "Tab") {
      ...
      if (matchingSuggestions.length > 0) {
        ...
        if (!activeSuggestion || activeSuggestion.field !== fieldName) {
          ...
          // 次の候補に移動
          const nextIndex =
            (activeSuggestion.index + 1) % matchingSuggestions.length;
          setActiveSuggestion({
            ...activeSuggestion,
            index: nextIndex
          });
          setFormData({
            ...formData,
            [fieldName]: matchingSuggestions[nextIndex]
          });
        }
      }
  ...
  const generatePoemText = (clothesName: string): string => {
    ...
    // 同じ衣装名のポエムを検索
    const matchingPoems = data.poems.filter(
      (poem) =>
  ...
  const generateIdIfPossible = async (updatedFormData: Partial<Poem>) => {
    if (
      updatedFormData.clothesName &&
      updatedFormData.idolName &&
      !updatedFormData.id
    ) {
      try {
      ...
      <div className="poem-list">
        {filteredPoems.map((poem) => (
          ...
          >
            ...
            <div className="poem-clothes">
              {poem.clothesTitle} / {poem.clothesName}
            </div>
            ...
            <div className="form-group">
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("idolName", e.target.value)
                  }
                  onKeyDown={(e) =>
                    handleKeyDown(e, "idolName", suggestions.idols)
                  }
                ...
                {activeSuggestion && activeSuggestion.field === "idolName" && (
                  ...
                  >
                    ...
                    {activeSuggestion.suggestions.length} 候補 (Tab: 次へ,
                    Enter: 確定, Esc: キャンセル)
                  </div>
                )}
              </div>
            ...
            <div className="form-group">
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("clothesName", e.target.value)
                  }
                  onKeyDown={(e) =>
                    handleKeyDown(e, "clothesName", suggestions.clothesNames)
                  }
                  onBlur={handleClothesNameBlur}
                  placeholder="Tabキーで補完"
                />
                {activeSuggestion &&
                  activeSuggestion.field === "clothesName" && (
                    <div
                      style={{
                        position: "absolute",
                        top: "100%",
            ...
            <div className="form-group">
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("clothesTitle", e.target.value)
                  }
                  onKeyDown={(e) =>
                    handleKeyDown(e, "clothesTitle", suggestions.clothesTitles)
                  }
                  placeholder="Tabキーで補完 (衣装名から自動コピー)"
                />
                {activeSuggestion &&
                  activeSuggestion.field === "clothesTitle" && (
                    <div
                      style={{
                        position: "absolute",
                        top: "100%",
            ...
            <div className="form-group">
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  onKeyDown={(e) =>
                    handleKeyDown(e, "text", suggestions.poemTexts)
                  }
                  maxLength={100}
                  placeholder="Tabキーで補完"
                />
